...
from mydb.db_utils import save_to_sqlite, init_db
...
def search():
    if request.method == 'POST':
        ...
        # 搜尋 raw_text
        query = "SELECT id, filename, raw_text, created_at FROM cards WHERE raw_text LIKE ?"
...
def list_cards():
    import sqlite3
    from mydb.db_utils import DB_PATH
...
def delete_card(card_id):
    ...
    # 查詢對應資料與圖片名稱
    cursor.execute("SELECT filename FROM cards WHERE id = ?", (card_id,))
    row = cursor.fetchone()
